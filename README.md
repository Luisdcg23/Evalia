...
- RSA-2048 nativo de .NET para la firma electrónica del PDF oficial, sin proveedor externo
...
- Integraciones externas descritas en la especificación pero fuera del alcance de esta entrega: SMS e integración con GIS/cartografía. El correo electrónico ya está implementado (SMTP vía MailKit): recuperación de contraseña, la aprobación o rechazo de un registro (con el motivo, si lo hay) y las notificaciones de `Notification.cs` (asignación, programación, revisión de informe, cierre de expediente) se envían también por correo al destinatario registrado. La firma electrónica también está implementada, pero con clave propia del sistema en vez de un proveedor externo (DocuSign y similares tienen costo): al generar el PDF oficial (`POST /api/evaluations/{id}/report/official`) se firma con RSA-2048 (clave persistida localmente, autogenerada en el primer arranque) y el metadato (`algoritmo_firma`, `firma_base64`, `huella_clave_publica`) queda junto al hash SHA-256 existente. `GET /api/evaluations/{id}/report/official/verify` recalcula el hash y valida la firma contra el binario tal como está hoy en el almacenamiento; `GET /api/signing/public-key` (anónimo) expone la clave pública para que un tercero verifique sin acceso al sistema.
